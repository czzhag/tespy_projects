...
def main():
	...
	doplot = True 
	inpath = '/home/data/cryo/20210302/20210302b3/test2'
		...
		#for row in [7]:
			for col in [0]:
				...
				for ib,bias in enumerate(biases):
					fn = '%s/fastdata_row%d_offset%d_bias%d'%(inpath,row,offset,bias)
					...
					if not len(y)==50000:
						...
						continue
					y  = np.reshape(y,(10,5000))
					y  = np.transpose(y)
					
					ym1= np.nanmean(y[:,[1,3,5,7,9]],1)
					ym2= np.nanmean(y[:,[2,4,6,8]],1)
			
					fsamp = tsc.info['freq']
					t  = np.array(range(5000))*1/fsamp*1000 # t in ms
					t0 = np.array(range(50000))*1/fsamp*1000	
			
					ds = 20 # ignore the spike
					try:	
						popt1,pcov1 = curve_fit(edge1,t[:-ds],ym1[ds:],bounds=([0,0,-5000],[100,5000,5000]))
						if popt1[0]==100 or popt1[1]<5.:
							popt1 = [np.nan, np.nan, np.nan]
						popt2,pcov2 = curve_fit(edge2,t[:-ds],ym2[ds:],bounds=([0,0,-5000],[100,5000,5000]))
						if popt2[0]==100 or popt2[1]<5.:
					...
					ax0.plot(t0,tsc.fb[0,col],color='b')
