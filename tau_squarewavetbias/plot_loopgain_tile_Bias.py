...
for tile in tiles:
	...
	Bbin = np.linspace(0,maxbin,51)
	Lpg_podr = np.full((len(podr),len(Bbin)-1),np.nan)
	for i,j in enumerate(podr):
		mce_row_podr=p.mce_row[j]
		mce_col_podr=p.mce_col[j]
		Lpg1 =abs(Lpg[mce_row_podr,mce_col_podr])
		B1  =plc.B[mce_col_podr]
		for k in range(len(Bbin)-1):
			Lpg_podr[i,k] = np.nanmedian(np.where((B1>=Bbin[k])*(B1<Bbin[k+1]), Lpg1, np.nan))

	ind_podr = np.where(np.sum(~np.isnan(Lpg_podr),1)>20)[0]
	
	Bi,chni = np.mgrid[0:maxbin:50*1j,0:len(podr[ind_podr]):(len(podr[ind_podr]))*1j]
	fig = plt.figure(figsize=(7, 6))
	cax = plt.pcolormesh(Bi, chni, np.log10(Lpg_podr[ind_podr]).T,vmin=0,vmax=2, cmap=plt.cm.rainbow)
	...
	print '%s/lpg_B_tile%d.png'%(outpath,tile)
	plt.savefig('%s/lpg_B_tile%d.png'%(outpath,tile))
