...
"""
# For N6/N7 SK run
# N6 was in col 0,1, N7 was in col 8,9
# N6 was with LPE. N7 had nothing above it.
#
# There were 5 open rs in this run: 01, 03, 05, 09, 19
# And cross-shorts 06x07, 07x08, 06x08 and 21x22.
# 
# They are most likely to have the same mapping as N3. (My guess)
#
# 20210423
...
class opts:
	...
	fn087 = 'LC_light_ba_FPU350mK_datamode1_sourceargon_LC_80K' 
	fn077 = 'LC_light_ba_FPU350mK_datamode1_sourceln2_77K'
	fitrange = {    'rnti_low': 500.00,
	                'rnti_hgh': 3000.00,
	                'sc_low': None,
	                'sc_hgh': None}
	cols = [0,1]
...
if opts.testbed in ['SK','sk']:
	...
	if opts.freq == 40:
        ...
        if opts.freq == 40:
        	...
        	import ba30_ModuleMapping_BA_N3 as minfo
        else:
        	print('***!!! wrong input')
        	print(opts)
else:
...
def main():
	...
	datafn087 = in_path + opts.fn087 + '/' + opts.fn087
	datafn077 = in_path + opts.fn077 + '/' + opts.fn077
		
	biasfn087 = datafn087 + '.bias'
	f087 = mce_data.MCEFile(datafn087)
	dname087 = os.path.split(datafn087)[0]
	...
	bias087 = np.loadtxt(biasfn087,skiprows=1)
	bias077 = np.loadtxt(biasfn077,skiprows=1)
	
	y087 = -1.0*f087.Read(row_col=True,unfilter='DC').data
	...
	for row in opts.rows:
		for col in opts.cols:
			...
			ibias087, ites087, rnti087, ksc087 = lc(bias087, y087[row,col], calib, fitrange, row, col)			
			ibias077, ites077, rnti077, ksc077 = lc(bias077, y077[row,col], calib, fitrange, row, col)
			rnti = (rnti087 + rnti077)/2.
			rnpsat = 1.3*rnti
			rr087, pp087, psat087 = pr(ibias087, ites087, calib, rnti, rnpsat, row=row, col=col, reasonableInorm=opts.reasonableInorm)
			rr077, pp077, psat077 = pr(ibias077, ites077, calib, rnti, rnpsat, row=row, col=col, reasonableInorm=opts.reasonableInorm)
			dpdt = (psat077 - psat087)/(87.5-77.)*1.0e12
			...
			if opts.doFigure:
				...
				ax = pl.subplot(1,2,1)
				pl.plot(ibias087*1e6,ites087*1e6,'r',
				...
				pl.plot(rr087*1.0e3,pp087*1.0e12,'r', rr077*1.0e3,pp077*1.0e12,'b') # pW-mOhms
				plt.axvline(x=rnpsat*1.0e3,color='g',linestyle='--')
				plt.scatter([rnpsat*1.0e3],[psat077*1.0e12], s=100, color='b', marker='+')
				plt.scatter([rnpsat*1.0e3],[psat087*1.0e12], s=100, color='r', marker='+')
				...
				if opts.ifoutputinMCE:
					if opts.testbed=='BA':
						fn = os.path.join(out_path_fig,'opteff_gcp%d_mcerow%02d'%(33*col+row, row) + '_mcecol%02d.png'%col)
					else:
